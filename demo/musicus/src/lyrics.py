...
from DLF.utils.helper import slack
...
def load_model(batch_size=3):
    ...
    K2S_eng_graph = tf.Graph()
    S2S_eng_graph = tf.Graph()
    S2S_kor_graph = tf.Graph()
    K2L_kor_graph = tf.Graph()

    K2S_eng_dir_path = 'data/preprocessed_inputs/K2S_ENG/input2~3_target7~10_20180305_6724955lines/'
    S2S_eng_dir_path = 'data/preprocessed_inputs/S2L_ENG/min7max10_20180305_4073481lines/'
    ...
    with K2S_eng_graph.as_default():
        K2S_eng_table = np.load(os.path.join(K2S_eng_dir_path, 'table.npy'))

        K2S_eng_model = K2S_Wrong_0307(K2S_eng_table,
                                       n_input=3,
                                       n_target=10,
                                       n_blocks=6,
                                       n_heads=10,
                                       loss_type='cross_entropy')
        K2S_eng_model.build_network()

        K2S_eng_infer = SingleInference(model=K2S_eng_model,
                                        keep_prob=0.9,
                                        dir_path=K2S_eng_dir_path,
                                        n_input=K2S_eng_model.n_input,
                                        n_target=K2S_eng_model.n_target,
                                        project_name='0305_ENG_K2S_onlyCE_Pretrained_data6M_noTargetMask_0309',
                                        use_local_wv=True,
                                        is_eng=True,
                                        )

    with S2S_eng_graph.as_default():
        S2S_eng_table = np.load(os.path.join(S2S_eng_dir_path, 'table.npy'))

        S2S_eng_model = S2L_CE_0307(S2S_eng_table,
                                    n_input=10,
                                    n_target=10,
                                    n_blocks=6,
                                    n_heads=10,
                                    loss_type='cross_entropy')
        S2S_eng_model.build_network()

        S2S_eng_infer = SingleInference(model=S2S_eng_model,
                                        keep_prob=0.8,
                                        dir_path=S2S_eng_dir_path,
                                        n_input=S2S_eng_model.n_input,
                                        n_target=S2S_eng_model.n_target,
                                        project_name='0305_S2S_onlyCE_min7max10_block6_data4M_pretrained_stepbasedoptimizer_0309',
                                        use_local_wv=True,
                                        is_eng=True,
                                        )
    ...
    return K2L_kor_infer, S2S_kor_infer, K2S_eng_infer, S2S_eng_infer, RNN_infer
